...
const Navigation = ({ isMobile, isOpen, toggleNav }) => {
    const navVariants = {
        hidden: { x: "-100%" },
        visible: { x: 0 },
    };
    ...
    return (
        ...
        >
            ...
            <motion.button
                ...
                className="relative px-4 py-2 sm:px-6 sm:py-3 lg:px-8 lg:py-4 text-sm sm:text-base lg:text-lg xl:text-xl font-bold uppercase rounded-xl overflow-hidden bg-gradient-to-r from-red-500 via-red-600 to-red-700 shadow-lg border-2 border-transparent hover:border-red-400 outline-none"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
            >
                <span className="absolute inset-0 bg-gradient-to-r from-red-500 to-red-700 opacity-15 animate-pulse"></span>
...
const VideoSection = ({ videoRef }) => (
    ...
    >
        <div className="relative rounded-full overflow-hidden border-4 sm:border-6 lg:border-8 border-white w-44 h-44 sm:w-60 sm:h-60 md:w-80 md:h-80 lg:w-[28rem] lg:h-[28rem] xl:w-[32rem] xl:h-[32rem] shadow-2xl">
            <video
                ref={videoRef}
                src={Lp}
